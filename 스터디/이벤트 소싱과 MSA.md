...
- **분산 트랜잭션**
  ...
  - 이 분산 트랜잭션은 대부분 성공할것이라는 낙관적 관점에 기반한다.
...
6. 분산 트랜잭션을 사용해 일관성을 유지하는 방법에 익숙한 개발자에게 보상 트랜잭션은 기술적으로 수용 불가한 방법일 수 있다.   하지만 도메인 전문가는 일관성을 달성하는데 소요되는 시간에 관대하거나 거의 신경쓰지 않는 경우가 많으므로 충분한 활용 가치가 있다.
   1. 이 말이 많이 와닿았다. 실무에서 나는 심각하게 받아들인 내용을 도메인 전문가는 별 문제 아닌듯이 말한 경험이 있다.
7. 이벤트로 다른 서비스와 협력하게되면 트랜잭션의 범위안에서 다른 서비스의 응답을 기다리는 경우가 많을 것 같은데.. 커넥션을 오래 잡고 있어 문제가 발생하는 일은 없을까?
8. 클린 아키텍처에서 중복된 코드를 진짜 중복과 우발적 중복으로 분류하고 중복을 제거할 때 주의해야 함을 설명한다.
9.  gson도 프로토콜 버퍼를 사용한다?
10. MSA 환경에서는 도메인별로 데이터베이스가 분리되어있을텐데, 이때 통계 데이터는 어떻게 생성하는것이 좋을까? 실시간 통계가 필요하다면?