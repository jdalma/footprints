...
- 경로 변수 : 가장 안정적이지만 엔드포인트가 다량 발생하고 옵셔널에 부적합하다.
- 쿼리스트링 : URL 쉐어 가능, 타서비스 간섭, 인코딩 문제, 쿼리스트링을 추가하거나 제거당할 수 있다.
- body, header : URL 쉐어로 표현되지 못함
- **해당 기능이 공유될 것인지 판별**
  - 공유 우선이라면 패스와 쿼리스트링만 사용
  - 설령 공유된다 하더라도 공유 전용 해쉬값을 생성할 수도 있음

# 스프린트

소프트웨어 개발 프로젝트에서 '시간 블록'을 나타내며, 스프린트의 범위를 정의하여 모든 사람이 함께 작업에 착수한다.  
2주 또는 2주 동안 **스프린트 계획 → 스프린트 검토 → 스프린트 회고** 순서로 진행된다.  

# 오류메세지

- 일관된 메세지로 보내야 사용측 오류 처리도 일관성을 지킬 수 있다.
- **에러 종류**
  - 주의 : 시스템은 허용하지만 보다 나은 방법을 알림
  - 경고 : 주의 강화판으로 실패가 될 가능성을 시사
  - 실패 : 시스템이 받아들이지 못함
  - 위협 : 시스템에 대한 공격으로 판정
  - 조치 : 사용자의 시도에 대해 이미 처리를 함 (블랙리스트에 추가하는)

# OAS 에러 메세지

- `message` : 현재 에러 상황을 포괄적으로 기술하지만, `errors`가 여러 의미를 포함하고 있기 때문에 힘듦
- `errors`
  - 배열로 되어 있어, 여러 개를 보낼 수 있음
  - 순서나 우선 순위, 중요도 등을 정해서 보낼 수 있다.
  - **서브스키마**
    - `path` : 잘못된 요청 필드를 가리키는 JSONPath
    - `message`: 기계가 해석하거나 다국어 리소스일 수 있음
    - `errorCode` : 응답 상태 코드가 아닌 사용자 정의 식별코드

# 실드 패턴

- 먼저 모든 값의 정합성(밸리데이션)을 계산함
- 정합성 확인에 실패하면 종료하기 때문에 이후 로직은 정합한 값만 이용함
- 정합성 계산과 비즈니스 로직이 섞이지 않아 보다 유지보수가 쉬워질 수 있음
- **정합성의 종류**
  - `필수`와 `선택`
  - 값 자체 정합 : 일치, 범위, 포함, 특수 조건
  - 값 간의 관계
  - 값이 다른 대상을 가리키는 경우
- OAS의 밸리데이터는 기본적으로 AND 조합임
  - 그룹화된 OR을 사용하려면 `object`와 `oneOf`의 조합을 써야함
  - **추상정의** : 필드의 밸리데이션을 매번 정의하는 것은 비효율적이기 때문에 사전에 정의된 필드를 `ref`로 재활용함
    - 그렇기에 사전에 그룹별로 정의된 다양한 타입을 만들어두게 된다.
    - `#/components/schemas/그룹체계1/그룹체계2/.../타입이름`

# API 버전

- **버전구분**
  - `major` : 사용자 측의 코드를 변경해야 함
  - `minor` : 사용자 측의 코드를 변경하면 더 좋아지지만 유지해도 되긴 함
- **전체 버전 vs 엔드포인트별 버전**
  - 전체 : 서비스 전체의 일관성이 유지됨
  - 엔드포인트 : 부분 업데이트를 기민하게 할 수 있음
  - **엔드포인트별 버전은 절대 쓰면 안된다.**

# 테스트 가이드

- **얼마나 많은 케이스를 테스트할 것인가?** 진리표 작성
  - 핸드폰 인증번호 발송여부 판정
    - wait - 인증번호 발송 후 대기 중? = 기존 인증 무효화 후 발송
    - out - 인증번호 발송 후 타임아웃? = 발송
    - ready - 인증번호 발송 전? = 발송
- **하나의 값을 어디까지 테스트할까?** 오른쪽 이두박근 (1~10 정수)
  - `Right` - 올바른가 = 1 ~ 10의 숫자
  - `Boundary` - 경계조건 = 1, 10
  - `Inverse` - 역관계를 확인하라 = 0, -1, 11, "a", 5.5, null 등
