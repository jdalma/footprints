...
## **HTTP/2는 HTTP/1.1의 성능 문제를 해결합니다. 하지만 한계점도 존재하는데요. 이 한계점에 대해서 설명해 주세요.**
...
1. HTTP/1.1과 HTTP/2를 어떻게 최적화해야 하는지 설명해 주세요.
2. HTTP/2 연결을 여러 도메인에 재사용할 수 있는 조건은 무엇인가요?
3. HTTP 상태 코드 421이 새로 도입된 배경은 무엇인가요?