...
## **HTTP/2 우선순위는 두 가지 방식으로 정의됩니다. 이 방식은 무엇인가요?**

HTTP/2에는 클라이언트가 요청의 상대적인 중요도(렌더링을 중단시키는 리소스 등)를 암시할 수 있게 하고자 우선순위 지점의 개념을 도입했다.  

> **스트림 우선순위: 힌트인가 명령인가?**  
> 스트림 우선순위는 요청자에 의해 전송되지만, 어떤 프레임을 전송할지 최종적으로 결정하는 것은 송신자다.  
> 따라서 우선순위는 암시나 힌트이고 우선순위를 무시하고 송신자가 가장 적절하다고 생각하는 순서대로 데이터를 보내는 것은 전적으로 송신자의 소관이다.  
> **"우선순위 전달은 ... 제안일 뿐이다."**  
>
> 서버 입장에서는 우선순위 지정 구현이 어려울 수 있지만 구현한 서버는 그렇지 않은 서버에 비해 성능 개선이 있을 것은 확실하다.

우선순위를 설정하는 두 가지 방식이 있다.

<h3>스트림 의존성</h3>

스트림은 다른 스트림에 의존할 수 있으므로 자신이 의존하는 스트림이 연결을 사용해 아무것도 보낼 필요가 없는 경우에만 리소스를 보내는데 사용해야 한다.

```
   index.html  -  main.css          -  main.js           -  image.jpg
   [stream:1]     [stream:3]           [stream:5]           [stream:7]
                  [dependency:1]       [dependency:3]       [dependency:5]
```

main.css는 index.html의 첫 의존성이며 가장 높은 우선순위로 보내져야 하고 그 다음은 main.js이며 마지막이 image.jpg이다.  
(다중 의존성은 지원하지 않는다.)  
이 의존성 계층 구조가 **의존하는 스트림이 부모 스트림을 중단시킨다는 뜻은 아니다.**  
예를 들어 main.css를 웹 서버에서 바로 사용할 수 없고 백엔드 서버에서 가져와야 한다면 main.js를 사용할 수 있다고 가정할 때 서버가 그 동안 main.js를 보낼 수 있다.  
만약 main.js도 사용할 수 없는 상황이라면 image.jpg를 보낼 수 있다.  
**스트림 우선순위 지정의 목적은 블로킹 매커니즘으로 작동하기보다는 연결을 가장 호율적으로 사용하는 것이다.**  
  
image.jpg를 보내는 도중에 main.css나 main.js를 사용할 수 있게되면 image.jpg의 전송을 중지하고 우선순위가 높은 자원을 먼저 보낸 후 image.jpg를 재개할 수 있다.  
아니면 준비가 되는대로 큐에 넣어서 순서대로 처리하는 방식일 수 있다.  
**선택은 서버의 몫이다.**  
  
**독점적 플래그를 통해서 같은 우선순위에 놓여있는 여러 스트림 중 특정 스트림에게 더 높은 우선순위를 줄 수 있다.**
  
<h3>스트림 가중치</h3>

동일한 부모 리소스에 의존하는 두 요청에 우선순위를 부여하는 데 사용되는 **가중치 부여** 가 있다.  
스트림에 가중치를 부여하면 동일한 의존성 수준에 있는 리소스에 서로 다른 가중치를 지정할 수 있기에 더 복잡한 시나리오도 가능해진다.  
가중치 부여가 독점적 플래그를 사용해 의존시키는 것과는 다르지만 비슷하다.  
...
critical.css가 전달되어야 main.css와 main.js가 각자 리소스의 50%를 얻는다.  
우선순위를 쉽게 지정하고 어떤 클라이언트는 PRIORITY 프레임을 사용해서 미리 적절한 우선순위를 갖는 더미 스트림을 설정한 다음 요청을 그 스트림에 매단다.  
아래의 nghttp가 우선순위를 이유로 연결 초반에 스트림 3,5,7,9,11을 설정하는 것과 같다.  
...
이렇게 설정된 스트림으로 효율적인 의존성 모델이 만들어졌으므로 해당 스트림에 원하는 리소스를 연결할 수 있다.  
  
하지만 우선순위의 지정은 서버와 클라이언트 양측에서 많은 구현이 필요하고 복잡하므로 현재 사용하고 있는 웹 서버는 어디까지 지원하는지 확인해야 한다.  

# 8장 HPACK 헤더

## **헤더 압축이 필요해진 이유는 무엇일까요?**

일반적으로 HTTP 헤더는 HTTP 본문보다는 상대적으로 작지만, 무시할 수 없는 크기가 반복적으로 작성된다.  
데이터 압축과 압축 해제에는 시간과 처리 능력이 필요하지만, 상대적으로 네트워크 요청을 보내기 전에는 압축하는 것은 가치가 있다.  
또한 HTTPS는 압축보다도 계산 비용이 많이 드는 암호화를 필요로 하는데, **먼저 압축한 다음 작은 크기의 데이터를 암호화하는 편이 낫다.**  
  
이 반복적인 헤더와 암호화 계산 비용을 줄이는 헤더 압축은 **HTTP/2의 전신인 SPDY에 헤더 압축이 들어가있었던 이유이며 HTTP/2의 핵심 요소이다.**  

## **무손실 압축과 손실 압축의 차에 대해서 설명해 주세요. 그리고 헤더 압축은 어떤 압축을 사용해야 하는지 설명해 주세요.**

<h3>손실 압축</h3>

세부 정보 중 일부는 필요하지 않기 떄문에 버려질 수 있으며 이 유형의 압축은 일반적으로 미디어에 사용된다.  
음악 파일, 이미지, 동영상은 데이터의 전체적 의미를 잃지 않으면서 크게 압축할 수 있다.  
손실 압축은 크기를 줄이는 것과 품질을 유지하는 것 사이에서 신중하게 균형을 잡는 것이다.  
  
**HTTP 헤더는 중요한 데이터이기 때문에 손실 압축을 선택할 수 없다.**

<h3>무손실 압축</h3>

무손실 압축은 데이터를 나중에 해제해야 할 때 손쉽게 다시 추가할 수 있는 반복적인 데이터를 제거하는 방식으로 동작한다.  
이를 수행하는데는 세 가지 방식을 사용한다.
1. 조회 테이블
2. 더욱 효율적인 인코딩 기법
3. 룩백 압축 방법

## **조회 테이블 압축 방법, 더 효율적인 인코딩 기법, 룩백 압축 방법에 대해서 설명해 주세요. 그리고 압축 방법들을 비교해서 언제 어떤 것을 사용해야 하는지 설명해 주세요**

<h3>조회 테이블 (lookup table)</h3>

길고 반복되는 데이터 비트를 참조로 바꾸는 것이다.  
압축 해제는 참조를 조회 테이블에서 가져온 원본 텍스트로 대체하는 것이다.  
테이블에는 형식에 따라 알려진 **표준 조회 테이블** 과 특정 텍스트에 대해 생성된 **동적 테이블** 이 존재하며 표준 테이블인지, 동적 테이블인지에 따라 압축 버전에 포함돼야 할 수도 있다.  
**조회 테이블은 값이 자주 반복되는 경우에만 유용하다.**  
  

<h3>더욱 효율적인 인코딩 기법</h3>

데이터를 표현하는데 좀 더 구체적인 방법을 사용할 경우 데이터를 좀 더 작은 크기로 표현할 수 있는 기법이다.  
1. 가변 길이 인코딩
2. 고정 길이 인코딩
3. (가변 길이 인코딩을 극단적인 수준으로 사용하는) 허프만 코딩

<h3>룩백(lookback) 압축</h3>

반복되는 텍스트를 현재 위치 관점으로 보고 참조하는 것을 말한다.  
반복되는 각 텍스트 조각은 압축 해제 프로그램이 반복되는 텍스트를 얼마나 되돌아가야 찾을 수 있는지와 얼마나 가져가야 하는지를 나타내는 참조로 대체된다.  
`(-20,3)`은 20문자를 되돌아가 3문자를 얻으라는 말이다.  



## **헤더를 통해서 브라우저와 서버가 압축 방법을 결정할 수 있습니다. 이 과정에 대해서 설명하고 왜 이렇게 만들어졌는지도 설명해 주세요.**

클라이언트가 요청 헤더에 `Accept-Encoding` 필드로 서버에게 자신이 지원하는 압축 알고리즘을 전달한다.  
서버는 전달받은 압축 알고리즘 중 하나를 골라 본문을 압축하고 응답 헤더에 `Content-Encoding`필드로 어떤 압축 알고리즘을 사용하는지 표시한다.  
이런 방식은 **새로운 압축 알고리즘이 도입될 수 있도록 허용하며, 클라이언트와 서버 양측에서 모두 지원되는 경우에만 사용되는 것이 보장된다.**  

## **HTTP/2가 자체적인 HTTP 헤더 압축 기법을 필요로 하는 이유는 무엇인가요?**

HTTP 헤더는 쿠키와 같은 민감한 데이터를 포함하므로 HTTP 본문과 같은 압축 기법은 다양한 공격을 통해 데이터가 유출될 수 있기 때문에 사용할 수 없다.  

> **CRIME(Compression Ratio Info-leak Made Easy)**  
>  네트워크 트래픽을 감시하며, 압축된 데이터의 크기 변화를 분석하여 중요한 정보(예: 세션 쿠키)를 추론하는 방식으로 작동합니다.  
> 공격자는 여러 가지 다른 입력을 시도하며, 압축된 페이로드의 크기가 어떻게 변하는지 관찰합니다.  
> 만약 공격자가 시도한 입력이 원본 메시지의 일부와 일치한다면, 압축 알고리즘에 의해 페이로드의 크기가 줄어들게 됩니다.  
> 이런 변화를 통해 공격자는 원본 메시지의 내용을 점차적으로 추론해낼 수 있게 됩니다.

CRIME(Compression Ratio Info-leak Made Easy)으로 시연한 위험떄문에 HTTP/2는 그런 유형의 공격에 영향을 받지 않는 다른 압축 방식을 사용해야 했다.  
HTTP 워킹 그룹은 조회 테이블과 허프만 코딩을 기반으로 하지만 HPACK이라는 새로운 사양을 만들었다.  
  
**HPACK은 HTTP/2 별도의 사양이다.** HPACK은 HTTP 사양과 달리 유연하지 않으며 확장할 수 있도록 설계되지 않았다는 점이다.  
**HPACK에는 61개의 공통 HTTP 헤더 이름에 대한 정적 테이블이 있으며 이 테이블은 요청 및 응답에 모두 사용된다.**  
그리고 세션동안 **62번째 위치에서 시작해 최대 SETTINGS 프레임에서 정의하는 SETTINGS_HEADERS_TABLE_SIZE 값까지 커질 수 있는 연결 수준의 동적 테이블을 갖는다.**  
  
헤더에 대한 테이블 항목 위치는 정적이지 않으며 향후 요청 모두에서 지속적으로 새로운 헤더가 테이블에 추가되는데 따라 증가한다.  
이러한 이유로 HEADERS와 CONTINUATION 프레임은 동적 테이블의 무결성을 유지하고자 순서를 맞춰 수신돼야 한다.  
**TCP가 프레임 수신 순서 정보를 보장하기 때문에 TCP 연결 마다 HTTP/2의 고유한 동적 테이블이 있게 된다.**  
  
# 9장 TCP, QUIC, HTTP/3

HTTP 연결을 생성하는데 큰 비용이 드는 작업은 TCP와 HTTPS 연결이다.  

## **TCP는 어떻게 무결성을 보장하나요?**

**HTTP는 TCP를 사용해 데이터가 안정적이고 순서를 맞춰 전달되는 것을 보장한다.**  
TCP는 두 끝점 사이의 연결을 허용하며 메시지 전달을 처리해서 메시지의 도착을 보장한다.  
메시지가 도착하지 않은 경우에는 재전송을 처리하며 어떤 다른 애플리케이션 계층(HTTP)으로 전달되기 전에 메시지가 올바르게 정돈되도록 보장한다.  
HTTP는 그 중 어떤 문제도 구현할 필요가 없으며 이 가정 위에 만들어졌다.  
  
**TCP는 각각의 TCP 패킷에 시퀀스 번호를 할당한 후 도착 시점에 패킷을 다시 정렬하거나(순서가 어긋나게 수신된 경우) 누락된 시퀀스 번호의 패킷을 다시 요청하는(패킷 누락이 감지된 경우) 방식으로 무결성을 보장한다.**  
그리고 CWND 기반으로 흐름제어를 하며 데이터의 최대량은 유기적으로 결정된다. 이 과정은 상당히 잘 동작하며 TCP/IP는 인터넷의 백본이였다.  

## **TCP의 비효율성은 무엇이고 HTTP/2에 어떤 영향을 미치나요?**

TCP가 동작하는 근본적인 방식은 5가지의 주요 문제를 야기한다.  

<h3>1. HTTP 연결 생성 시 설정 지연</h3>

![](./imgs/httpsHandshake2.png)

전송자와 수신자가 사용할 시퀀스 번호는 연결 시작 시점에 서로 동의해야 하고 HTTPS 설정에 결합된 핸드셰이크는 첫 HTTP 메시지가 전송되기 전까지 상당한 **설정 지연을 발생시킨다.**  
HTTP/2가 연결의 숫자를 크게 줄일 순 있긴 하지만 지연이 완전히 사라지진 않는다.

<h3>2. TCP의 혼잡 제어 비효율성</h3>

연결을 맺은 다음에도 TCP의 비효율성은 다른 성능 문제를 일으킨다.  
일차적으로는 **모든 TCP 패킷이 순서에 따라 도달하게 될 것이라는 TCP의 본질이 보장되기 때문이다.**  
이 문자을 지키려면 프로토콜, 특히 혼잡 제어에 몇 가지 고려 사항을 내장해야 한다.  
  
**혼잡 제어는 네트워크가 새로운 패킷을 보내는 것보다 손실된 패킷을 재전송하는데 더 많은 시간을 소비하는 경우에 네트워크 붕괴를 방지하는 것을 목표로 한다.**  
이러한 문제를 방지하고자 다양한 혼잡 제어 기능으로 TCP를 개선하여 안정성을 가져왔지만 비효율성도 가져왔다.  
  
**TCP 느린 시작**  
- 해당 메커니즘은 TCP가 네트워크를 넘치게 하거나 잠재적으로 위험에 빠뜨리지 않을 정도의 TCP 네트워크 최적 처리량을 찾는다.
- 낮은 속도에서 시작해 최대 수용 가능 속도까지 높이는 신중한 알고리즘이다.
- 세그먼트 크기가 1460바이트인 세그먼트 10개, 즉 대략 14KB에서 2배씩 증가한다.
- 윈도우 사이즈가 최대 수용량에 다다르고 패킷 손실이 없다고 가정하면 TCP 혼잡 제어는 혼잡 회피 단계에 들어가며 이 단계에는 훨씬 느린 선형적 속도로 증가한다.

> **TCP 느린 시작은 그렇게 느린가?**  
> TCP 느린 시작의 기하급수적 본질 때문에 대부분의 정의상으로 느린 시작은 느리지 않다.  
> 사실 혼잡 회피 단계의 TCP 증가 속도가 훨씬 느리다.  
> 느린 시작 방식은 서버가 보낼 수 있는 최대 크기에서 시작해 크기를 줄이는 것보다는 확실히 느리다.  
> **모든 TCP 연결은 이 과정을 거치므로 프로토콜은 처음에 느리다**

  
<h3>3. 유휴 연결이 저하시키는 성능</h3>

> **TCP의 유휴 단계**  
> 양쪽 시스템이 연결은 되어 있지만 특정한 데이터 전송이 없는 상태를 의미하며 데이터 전송이 필요하지 않거나, 네트워크 혼잡이나 다른 이유로 인해 데이터 전송을 일시 중지해야 할 때 발생한다.  

TCP 느린 연결 알고리즘은 연결의 시작 지점에 지연을 일으킬 뿐만 아니라 **연결이 유휴 상태일 때도 지연을 일으킨다.**  
유휴 기간이 지나고 나면 네트워크 상태가 변경됐을 수 있으므로 TCP는 CWND 크기를 다시 제한하고 느린 시작 매커니즘을 다시 시작해 최적의 CWND 크기를 다시 탐색한다.  
  
연결이 재사용되면 최소한 **TCP 핸드셰이크와 HTTPS 핸드셰이크는 반복하지 않긴 한다.**  

<h3>4. 패킷 손실이 저하시키는 TCP 성능</h3>

TCP는 패킷 손실이 감지되면 수신 서버의 수용량에 문제가 있다고 가정하여 CWND를 반으로 나누기 때문에 통신량이 급격히 떨어진다.  

![](./imgs/packetLoss.png)

하지만 패킷 손실은 여러 가지 이유로 발생할 수 있으며, CWND를 절반으로 나누는 방식은 적절치 못하다.  
예를 들어, 모바일 네트워크는 유선 연결보다 안정적이지 못하며, 네트워크 수용량이 얼마나 되든지 간에 관계없이 임의로 패킷을 잃어버릴 수 있다.  
따라서 패킷 손실이 순수하게 혼잡 때문에 발생한다고 가정하는 것은 문제가 될 수 있다.  
  
**이 문제는 단일 연결을 사용하는 HTTP/2의 경우 모든 리소스가 문제를 겪기 때문에 특히 더 심각하다.**  

<h3>5. 패킷 손실은 아이템을 큐잉되게 만든다.</h3>

HTTP/2 다중화를 사용하면 동일한 TCP 연결에 여러 스트림의 요청을 병렬로 진행 상태에 둘 수 있다.  

스트림 5,7,9 번에 대한 자원이 클라이언트에게 전달되는 도중에 5번 스트림의 패킷이 사라졌다고 가정해보자.  
그러면 아래와 같이 5번 스트림에 대한 패킷은 큐의 끝에 추가된다.  

![](./imgs/retransmission.png)

TCP가 순서를 보장하기 때문에 이 응답은 위와 같이 반드시 큐잉돼야 하므로, script.jss와 image.jpg는 완전히 다운로드 됐지만 사용할 수 없다.  
  
요점은 **HTTP/2가 여러 연결을 사용하는 HTTP/1.1에서 없었던 제약을 추가한다는 것이다.**  
더 나쁜 점은 제한된 TCP 버퍼 크기 때문에 연결이 순서에 어긋난 모든 패킷을 큐에 넣지 못할 경우 일부 패킷을 버려서 그 패킷들도 재전송되게 만들 수 있다.  
HTTP/2는 HOL 블로킹 문제를 HTTP 수준에서 해결했는데, **다중화를 사용하면 하나의 지연된 응답 때문에 HTTP 연결이 다른 리소스에 사용되지 못하는 상황이 생기지 않기 때문이다.**  
**그러나 하나의 스트림에서 손실된 패킷은 뒤로 미뤄질 필요가 없는 다른 모든 스트림도 블로킹하기 때문에 TCP 수준에서는 HOL 블로킹이 여전히 존재한다.**  
  
## **혼잡 제어란 무엇이고 TCP의 혼잡 제어는 왜 비효율적인가요?**

혼잡 제어는 네트워크의 혼잡 상태를 관리하고 제어하는 메커니즘이다.  
TCP의 혼잡 제어는 패킷 손실이 일어나면 수신 서버의 수용량에 문제가 있다고 가정하여 윈도우 사이즈를 반으로 나누기 때문에 통신량이 급격히 떨어진다.  

## **TCP의 느린 시작이 HTTP 프로토콜에 미치는 영향은 무엇인가요?**

클라이언트와 서버간 TCP 통신간의 최적 처리량을 찾는 것이다.  
낮은 속도에서 시작해 최대 수용 가능 속도까지 높이는 신중한 알고리즘이지만 처음에는 느릴 수 밖에 없다.  

## **패킷 손실 상황에는 HTTP/2가 오히려 HTTP/1.1보다 더 느릴 수 있습니다. 그 이유를 설명해 주세요.**

패킷 손실이 감지되면 TCP는 윈도우 사이즈를 절반으로 줄이게 되는데 HTTP/1.1의 경우 연결을 단일 요청으로 사용하기 때문에 전송량이 단일 요청에만 영향을 주게된다.  
하지만 HTTP/2의 경우 단일 연결 다중 요청으로 사용하기 때문에 특정 스트림의 패킷 손실로 인해 윈도우 사이즈가 줄어들면 해당 연결을 통해 통신되는 다른 스트림에게도 영향을 주기 때문에 HTTP/1.1보다 느릴 수 있다.  

## **HTTP/2에서 스트림을 전송하는 중에 패킷 손실이 일어난다면 어떤 일이 일어나는지 설명해 주세요.**

HTTP/2 통신에서 특정 스트림에 대한 패킷이 손실되면 손실된 패킷은 큐의 마지막에 담기게 된다.  

## **TCP를 최적화하는 방법에는 무엇이 있나요?**

HTTP 프로토콜의 비효율성은 HTTP/2로 제거됐지만, 이제 다른 곳에 존재하는 성능상 병목현상이 더 분명히 드러나게 됐다.  
HTTP HOL 블로킹이 HTTP/2에서는 다중화 덕분에 더 이상 문제가 되지 않는다.  
하지만 TCP HOL 블로킹은 문제가 되고, 특히 패킷 손실이 있는 환경에서는 더욱 문제가 된다.  
  
1. **운영체제 업그레이드**
   - 보통 저수준 운영체제가 TCP를 제어하며, 운영체제 외부에서 이를 변경할 기회가 별로 없다.
   - 리눅스에서 대부분의 TCP 설정을 `/proc/sys/net/ipv4`에서 확인할 수 있다.
   - `cat /proc/sys/net/ipv4/tcp_slow_start_after_idle`
   - 느린 시작의 창 사이즈는 수년에 걸쳐 2와 4로 증가했고 리눅스 커널 2.6.39에서는 4에서 10으로 증가했다.
2. **선택적인 확인 사용하기**
   - 선택적 확인을 사용하면 다른 패킷이 버려진 경우 재전송을 피하고자 순서에 맞지 않더라도 TCP가 패킷에 대한 확인을 보낼 수 있다.
   - 패킷 1 ~ 10이 전송됐지만 패킷 4가 버려졌다면 패킷 1 ~ 3과 5 ~ 10을 확인할 수 있다.
   - `cat /proc/sys/net/ipv4/tcp_sack`로 조회할 수 있다. 1(켜짐)
3. **느린 시작의 재시작 비활성화하기**
   - TCP 연결의 속도는 유휴 기간이 지난 후에는 네트워크 상태가 변경됐을 수 있어 창 사이즈는 줄어든다.
   - 이 설정은 보통 기본적으로 활성화되지만 최적의 설정이 아닐 수 있다.
   - `cat /proc/sys/net/ipv4/tcp_slow_start_after_idle`
4. **TCP 빠른 열기 사용하기**
   - 트래픽의 첫 패킷을 TCP의 3방향 핸드셰이크의 첫 SYN 부분과 함께 보낼 수 있다.
   - 이 방식은 TCP와 연관된 설정 지연 일부를 방지한다.
   - 보안상의 이유로 이 패킷은 TCP의 첫 연결이 아니라 재연결에서만 전송될 수 있으며, 클라이언트와 서버가 모두 지원해야 한다.
   - 하지만 많은 브라우저나 운영체제, 웹 서버에서 지원되지는 않고 있으니 지원하는지 확인해야 한다.
   - `cat /proc/sys/net/ipv4/tcp_fastopen`
   - ![](./imgs/fastOpen.png)
5. **혼잡 제어 알고리즘 PRR과 BBR 사용**
   - TCP에는 패킷 손실이 발생하는 경우 TCP가 반응하는 방식을 제어하는 다양한 혼잡 제어 알고리즘이 있다.
   - 대부분의 TCP 구현에서는 패킷이 손실될 때 혼잡 제어 창을 절반으로 줄이는 CUBIC 알고리즘을 사용한다.
   - 더 개선된 PRR (Proportional Rate Reduction, 비례 비율 감소), BBR (Bottleneck Bandwidth and Roundtrip propagation time, 병목 대역폭 및 라운드 트립 전파 시간)
   - `cat /proc/sys/net/ipv4/tcp_congestion_control_cubic`으로 확인할 수 있다

## **QUIC은 HTTP/2의 어떤 문제를 해결하려고 하나요?**

[QUIC 성능상 이점](https://blog.chromium.org/2015/04/a-quic-update-on-googles-experimental.html)  
앞에서 TCP의 문제들을 확인해보았는데, TCP가 HTTP 하부 프로토콜로 적합한지 HTTP의 요구 사항에 맞춰 설계된 새 프로토콜이 적합한지 생각해볼 수 있다.  
**QUIC은 구글에서 발명한 UDP 기반의 새로운 프로토콜로 이번 장에서 언급한 비효율성 중 다수를 해결하고자 전통적인 HTTP 스택의 TCP와 다른 부분을 대체하는 것을 목표로 한다.**  
HTTP/2는 TCP와 유사한 개념(패킷과 흐름 제어 등)을 몇 가지 도입했지만, QUIC은 이러한 개념을 한 단계 위로 끌어올려서 TCP를 대체한다.  
  
HTTP/2는 근본적인 병목 일부를 제거함으로써 프로토콜의 성능을 개선한 것은 확실하다. 하지만 HTTP의 모든 문제를 해결하려 한것은 아니다.  
QUIC은 TCP HOL 블로킹, 더 완벽한 암호화, 개선된 연결 설정, 연결 마이그레이션 등 HTTP/2가 해결하지 않았던 문제 일부를 다루는 것을 목표로 한다.  
아래의 5가지를 염두에 두고 설계됐다.  
...
1,2,3번은 TCP(및 HTTPS)의 명백한 단점을 보완한것이다.  
4,5번은 위의 문제를 개선을 넘어 보완한 것들이다.  

> QUIC은 HTTP 보다 훨씬 더 넓고 포괄적인 전송 계층 프로토콜로 자리 매김했고, HTTP/3은 QUIC의 그저 한 가지 사용 사례일 뿐이며 많은 면에서 QUIC은 HTTP/2가 아니라 TCP의 후속 제품이지만, TCP/2 라는 이름으로 불리지 않고있다.  
> 이는 HTTP/3 이라는 이름이 더 큰 QUIC 프로토콜과 QUIC의 HTTP 부분을 차별화하고자 사용되는 이유다.  
  
<h3>포워드 오류 수정</h3>

포워드 오류 수정 (FEC, Forward Error Correction)은 QUIC 패킷의 일부를 인접 패킷에 포함시켜 패킷 재전송의 필요성이 줄어들기를 기대한다.  
**단일 패킷만 버려진다면 성공적으로 전달된 패킷에서 해당 패킷을 재조립할 수 있어야 한다는 발상이다.**  
  
이 과정은 네트워크 수준의 [RAID 5](https://www.easeus.co.kr/storage-media-recovery/raid-5.html)와 비교된다.  
**QUIC은 중복과 오버헤드 비용을 추가하지만 HTTP가 배달 보장을 필요로 한다는 사실을 감안하면 그 이득은 작은 오버헤드를 들일만하다.**  

<h3>연결 마이그레이션</h3>

**연결 마이그레이션의 목표는 네트워크 간 연결 이동을 허용해서 연결 설정 오버헤드를 줄이는 것이다.**  
TCP에서의 연결은 양쪽의 IP 주소와 포트가 변경되면 새로운 TCP 연결을 설정해야 한다. 세션이 지속되는 동안 IP 주소가 바뀔 일은 낮았기 때문에 괜찮았다.  
이제는 다수의 네트워크가 사용되기 때문에 QUIC을 사용하여 집에서 와이파이로 세션을 시작한 후 세션을 다시 시작하지 않고도 모바일 네트워크로 이동할 수 있도록 개선한 것이다.  
대역폭을 늘릴 수 있는 **다중 경로** 라는 기법으로 하나의 QUIC연결을 위해 와이파이와 모바일 네트워크 모두를 동시에 사용할 수도 있다.  

## **왜 QUIC은 TCP를 개선하지 않고 UDP위에서 구현했나요?**

![](./imgs/quicStack.jpg)

QUIC은 TCP보다 많은 것을 대체한다. **QUIC은 TCP가 전통적으로 제공하던 것의 대부분인 `설정`, `신뢰성`, `혼잡 제어 부분`과 HTTP/2의 `흐름 제어` 와 `헤더 압축` 까지 대체한다.**  
QUIC은 스트림 수준 혼잡 제어와 인터넷에 영향을 주는 HTTPS 핸드셰이크, 제한된 패킷 유실, 연결 마이그레이션등을 개선하는데 목표를 둔다.  
  
**QUIC은 연결 계층과 암호화 계층을 동시에 수행함으로써 단일 라운드 트립 연결 설정을 목표로 한다.**  
그러기 위해 TCP와 TLS에 추가된 개념인 TCP 빠른 열기와 TLSv1.3 등 많은 것을 사용한다.  
HTTP/2를 대체하지는 않지만 전송 계층의 일부를 넘겨받아 더 가벼운 HTTP/2 구현이 최상단에서 실행되게 한다.  
(HTTP/2에 도입된 개념인 다중화된 스트림, 헤더 압축, 서버 푸시 그대로 존재한다.)  
  
**프로토콜은 여전히 다중화된 스트림 기반 바이너리 프로토콜이며, 이런 변화를 이루고자 저수준에서 사용된 일부 세부 사항은 이제 HTTP/2가 아니라 QUIC의 책임이다.**  
QUIC을 통한 HTTP를 **HTTP/3** 이라고 칭한다.  
  
<h3>UDP의 성격과 QUIC이 UDP 기반인 이유</h3>

TCP는 재전송, 혼잡, 흐름 제어를 포함하는 신뢰성을 구현하기 때문에 TCP HOL 블로킹 문제가 발생한다. 이 문제는 HTTP/2에 비효율성을 가져온다.  
**UDP는 TCP와 비슷한 포트 개념이 있으므로 여러 개의 UDP 기반 서비스를 동일한 컴퓨터에서 실행할 수 있다.**  
또한 UDP에는 UDP 패킷의 무결성을 확인할 수 있도록 **선택적인 체크섬** 도 있다.  
이 두 개의 기능을 제외하고서는 특별한것이 없기에 만약 신뢰성, 순서,혼잡 제어가 필요하다면 애플리케이션에서 만들어야 한다.  
  
UDP는 동영상과 같이 서비스에 지나친 문제를 일으키지 않고 일부 프레임을 버릴 수 있는 제대로 전달되는 것을 보장하지 않아도 되는 애플리케이션에서 사용된다.  
패킷이 유실된 경우 자동으로 재전송되지 않을 것이고 순서가 어긋나게 도착해도 보장하지 않는다.  
HOL 블로킹이 없기 때문에 HTTP/2와 같이 다중화된 프로토콜에도 매우 적합하다.  

<h3>TCP를 개선하지 않는 이유</h3>

TCP도 여전히 발전중이고 문제는 추가적인 개선으로 제거할 수 있을텐데 왜 TCP를 개선하여 사용하지 않을까?  
주된 단점은 개선의 구현 속도다. **TCP는 거의 항상 운영체제에 빌트인될 정도의 핵심 프로토콜이며, 서버 측에 이를 구성하기 위한 약간의 변경이 가능하고 일부 개선도 이뤄질 수 있다. 하지만 대부분의 TCP 개선에서 이득을 얻으려면 운영체제를 업그레이드 해야 한다.**  
TCP 빠른 열기는 큰 이득을 제공하지만 아직까지 대다수의 인터넷 브라우저나 서버에서 사용되지 않는 것과 같이 혁신이 널리 퍼지는데 많은 시간이 든다.  
  
그리고 인프라는 TCP와 같은 프로토콜 사양에 대한 의존이 있으므로 그 사양을 깨면 문제가 발생할 수 있다.  
이러한 문제를 **프로토콜 경직화** 라고 한다.  
이런 이유로 TCP에서 떠남으로써 QUIC은 더 큰 자유를 얻었다.  

<h3>IP를 직접 사용하지 않는 이유</h3>

전송 계층의 요구 사항은 가볍기 때문에 QUIC 설계자들은 IP 계층위에 만들수도 있었다.  
(IP는 출발지 및 목적지 IP 주소일 뿐이다. 그 외의 모든것은 그 위에서 만들 수 있다.)  
  
하지만 IP 패킷에 직접 접근 권한을 얻는 애플리케이션이 거의 없기 때문에 프로토콜이 운영체제 수준에서 구현돼야 한다.  
또한 QUIC은 특정 애플리케이션을 대상으로 해야 하므로 UDP가 갖는 포트를 필요로 한다.  
HTTP를 사용하는 어떤 프로그램이라도 QUIC을 통해 별도의 HTTP 연결을 열 수 있다.  
IP를 직접 사용하려면 모든 QUIC 패킷을 읽어 각각에 적절히 라우팅해야 한다.  
  
<h3>UDP 이점</h3>

**UPD도 커널에 구현된 기초적 프로토콜이다. 그 위에 만들어진 모든 것은 사용자 공간 (user space)의 애플리케이션 계층에서 만들어져야 한다.**  
커널 밖에 있으면 애플리케이션을 양측에 배포해 빠르게 적용시킬 수 있다.  
  
구글은 클라이언트와 서버, 연결의 양쪽을 제어할 수 있고, 기존 프로토콜 변경하지 않고 가볍고 제한된 UDP를 사용함으로써 QUIC을 빠르게 적용할 수 있었다.  
  
UDP는 일반적인 프로토콜이지만 TCP만큼 아니다 그리고 UDP에 문제가 없지는 않다.  
UDP는 종종 방화벽과 미들웨어에 의해 차단되고 QUIC의 초기 측정값은 동등한 TLS/TCP 기반 서버보다 CPU를 3.5배까지 사용함을 보여준다.  
결과는 여전히 UDP가 더 비싼 프로토콜이며 QUIC이 커널 밖에 있는 동안은 어쩔 수 없을 것이다.  

## **QUIC의 표준에 대해 설명하고 HTTP/2와 QUIC의 차이점에 대해 설명해주세요**

두 개의 표준으로 구성된(HTTP/2와 HPACK) HTTP/2와 달리 **QUIC 표쥰은 하나가 아니라 여섯 개다.**  

1. **QUIC 불변 요소** : 향후 버전에서 변경되지 않을 QUIC의 요소
2. **QUIC 전송** : 핵심 전송 프로토콜
3. **QUIC 복구** : 손실 감지와 혼잡 제어
4. **QUIC TLS** : TLS 암호화가 QUIC에서 사용되는 방식
5. **HTTP/3** : 크게 HTTP/2를 바탕으로 하며 약간의 변경이 있음
6. **QUIC QPACK** : QUIC의 HTTP를 위한 헤더 압축

IETF 워킹 그룹에 의해 표준 작업이 진행 중이기 때문에 사양 내용이나 사양 수가 변경될 수 있다.  
주목해야 할 중요한 점 중 한 가지는 QUIC이 범용 프로토콜로 의도됐다는 것이다.  
현재 HTTP가 QUIC의 주요 사용 사례일 뿐이지 이 프로토콜은 잠재적인 다른 사용 사례를 염두에 두고 설계되고 있다.  
  
QUIC은 HTTP/2를 기반으로 하기 때문에 QUIC이 표준이 되더라도 HTTP/2에 대한 학습은 도움이 될 것이다.  
하지만 하부 UDP 프로토콜을 포함하는 몇 가지 주요 차이점이 있다.  
  
1. **QUIC과 HTTPS**
   - HTTPS는 QUIC에 내장돼 있고 HTTP/2와 달리 암호화되지 않은 HTTP연결에 QUIC을 사용할 수 없다.
2. **QUIC 연결 설정**
   - HTTP/2는 ALPN, Upgrade 헤더, 사전 지식, HTTP Alt-Svc 헤더, HTTP/2 프레임을 포함해 HTTP/2 프로토콜 협상을 위한 몇 가지 방식을 확립했지만 모든 방식은 처음에 TCP의 사용을 가정한다.
   - QUIC은 UDP에 기초하기 때문에 웹 서버에 연결하는 웹 브라우저는 TCP에서 연결을 시작해 QUIC으로 업그레이드 해야 한다.
   - 이 과정은 TCP를 통한 HTTP에 대한 의존성을 가져오며, 따라서 QUIC의 핵심적인 이점을 무효화한다.
3. **QPACK**
   - 헤더 압축에 사용되는 HPACK은 HTTP 헤더 프레임이 순서에 맞춰 수신됨을 보장하고자 TCP의 보장 특성에 의존해 동적 테이블이 양쪽에서 올바르게 유지될 수 있게 한다.
   - HPACK은 연결 수준에서 순서에 맞는 패킷 전달 보장이 필요하기 때문에 HOL 블로킹을 다시 가져온다.
   - QPACK은 HPACK의 이러한 단점을 보완하기위해 등장했다.

# 10장 HTTP가 나아가는 방향

## **HTTP/2가 해결하지 못한 문제에 대해서 설명해 주세요.**

1. **SPDY에 대한 논쟁**
   - SPDY가 HTTP/2에 대한 유일한 표준인가?
   - HTTP가 전체적으로 개선될 수 있는 방법을 더 폭넓게 검토해야 했다라는 의견이 많다.
2. **HTTP의 개인 정보 문제와 상태**
   - HTTP는 상태 없는 프로토콜로 설계됐지만 요즘 어플리케이션과 웹은 상태를 필요로 한다. 이 문제를 해결하기위해 쿠키를 사용하는 것
3. **HTTP와 암호화**
   - 암호화는 프로토콜에서 강제하는 것이 옳지 않은 것인지?

## **쿠키란 무엇이고 어떤 문제가 있는지 설명해 주세요.**

Secure, HttpOnly 플래그 등 옵션이 추가되긴 했지만 쿠키는 기본적으로 안전하지 않다.  
쿠키는 모든 요청마다 전송되기 때문에 불필요한 데이터를 보내 정보를 유출하는 위험이 있다.  
그리고 로딩 중인 사이트만 쿠키를 사용할 수 있는 것이 아니라 사이트를 위해 웹 브라우저가 로드하는 모든 리소스도 쿠키를 사용할 수 있는 문제도 있다.  
  
1. **SameSite 쿠키** : 이는 쿠키에 대한 보안 개선으로, `CSRF(Cross-Site Request Forgery)` 공격을 방지하는 데 도움을 준다. SameSite 쿠키는 브라우저가 다른 사이트에 대한 요청을 할 때 쿠키를 보내지 않도록 제한한다.
2. **HTTPOnly 쿠키** : 이는 `XSS(Cross-Site Scripting)` 공격을 방지하기 위한 방법으로, 쿠키를 자바스크립트를 통해 액세스할 수 없도록 하는 속성이다.
3. **Secure 쿠키** : 이는 쿠키가 HTTPS를 통해서만 전송되도록 하는 속성으로, 중간자 공격(MITM)을 방지하는 데 도움이 된다.

## **HTTP/2의 계층화 위반에 대해 설명해주세요**

![](./imgs/layer.png)

HTTP/2는 전통적인 계층에서 이탈해 TCP의 많은 특성을 채택했다.  
이 계층간 분리가 되어있는데 예를 들어 HTTP가 파일 형식의 통신을 허용하지만, 계층 6은 이러한 기술 중 어떤 것에도 직접 연결되지 않는 것 처럼 말이다.  
  
**HTTP/2는 더 이상 HTTP/1이 그랬던 것처럼 최상위 애플리케이션 계층에만 연결되지 않는다.**  
멀티플렉싱과 흐름 제어같이 전통적으로 전송 계층의 소관으로 여겨졌다. HTTP/2는 TCP가 이러한 개념을 다루는 것을 대체하기보다는 `반복하기 때문에` TCP는 HTTP/2에 더 가까우며, 전통적으로 HTTP라고 간주되던 것을 보완한다.  
**따라서 새로운 바이너리 프레이밍 계층은 여러 계층에 걸쳐있다.**  
  
**QUIC은 애플리케이션 전송 프로토콜(HTTP)과 네트워크 전송 프로토콜(TCP와 UDP) 사이의 계층화가 더 분명한 상태로 되돌아가는 것을 추구한다.**  
이 모델에서 HTTP/3은 HTTP 프레임 유형을 보여주며 HTTP 트래픽에 프레임 유형이 사용될 방식을 알려주지만, 전송 계층 정보(다중화 스트림 관리 등)는 QUIC 자체에 남겨둔다.  

## **앞으로 새로운 HTTP 메서드가 추가될까요?**

HTTP/1.1 이후 주요 메서드가 확장되지 않았다.  
HTTP 메서드는 비교적 추가하기가 쉽지만 필요하지 않으며, 대부분의 요구사항은 POST가 대신하거나 HTTP 헤더를 추가하여 해결할 수 있다.  
새로운 메서드가 추가될 수 있긴 하겠지만 큰 영향력을 끼칠일은 없을 것이다.  

## **앞으로 새로운 HTTP 헤더는 더 많이 추가될까요?**

HTTP/2는 새로운 [설정 값](https://www.iana.org/assignments/http2-parameters/http2-parameters.xhtml#settings)과 프레임 유형으로 새로운 능력을 추가한다.  
HTTP 헤더를 통해 기능을 확장하는 것은 상대방이 이해하기를 바라는 것이지만 설정 값을 통해 확장하는 것은 HTTP/2 연결 전체에 대해 동작을 제어하기 때문에 더 나아졌다고 볼 수 있다.  
결국 HTTP 헤더는 메시지 레벨(요청과 응답)에서 동작을 제어하고 HTTP/2 설정값은 연결 레벨(TCP 연결)에서 동작을 제어한다는 것이다.  
(새로운 프레임 유형으로 프로토콜을 확장할 수도 있다.)  
  
반면에 HTTP/1에서는 HTTP 헤더의 사용이 증가했고, 앞으로도 계속 증가할 것으로 예상된다.  
(HTTP/2에서 가상 헤더의 사용을 금지하지만 새로운 사양으로 추가될 수 있긴 하다.)  