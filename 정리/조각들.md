...
4. [ ] 개발 프로세스와 일정 산정 가이드가 나와 굉장히 비슷하다
   ...
   2. [ ] 저희는 분석 -> 설계 -> 개발 순으로 진행되는데 분석,설계 일정은 대략적으로 산출하고, 개발 일정을 확실하게 산출하는 것 같다.
   3. [ ] 개발에서 분석이나 설계 단계에서 놓친 사항들이 발견되곤 하는데, 혹시 팁같은 것이 있을까요? 문서 작성 툴이라던지, 작성 방법이라던지?

> 후에 발견한 이슈 대응은 괜찮다.  
> 구현의 목표, 어떤 임팩트를 얻게되는지, 문제 상황  
> 개발 방향성에 대해 장단을 다 따지기의 과정은 필요하다  
...
1. [ ] 큰 트래픽을 다루기 위해 오토 스케일링도 적용되어 있을까요?
   1. [ ] 서비스 오토 스케일링이 적용되어 있다면 DB에 부하는 어떤 식으로 해결하시나요?
   2. [ ] 읽기 작업이 많을 것 같은데 읽고 쓰는 DB가 분리되어 있나요?
   3. [ ] 오토 스케일링이 편한 NoSQL을 적극적으로 사용하시나요?

> BFF : 모바일에 필요로하는 응답과, 웹에 필요로 하는 응답과 구분할 때  
> + Materialized view이 개념에 속할 것 같다.

2. [ ] Materialized view 서버는 캐시 서버로 보이던데 개발이 진행되고 있는건가요?
...
3.  [ ] 이벤트 주도 아키텍처를 구축하는 것이 목표라고 알고있는데, 현재는 서비스간에 통신은 동기로 이루어지고 있는건지?
   1.  [ ] 동기로 통신하면서 문제가 많이 발생하는 것 같다.
4.  [ ] 이벤트 주도 아키텍처를 적용이 진행 중인가요?
   1. [ ] 진행 중이라면 어떤 작업을 하고 계신가요?
   2. [ ] 카프카를 써보시니 어떠셨나요?
   3. [ ] 비동기 프로그래밍 어렵기도 하겠지만 재밌을 것 같다.

> POC : 이 컨셉으로 동작하는지 검증하는 
...
- Armeria Server Thread가 생성되는 기준에 대한 이해
  ...
  - 마지막 RST flag를 보내면서 서버에서 Socket이 닫히고 EventLoop와 Channel이 소멸된다고 생각하고 있다
  - **근데 이렇게 되면 스레드의 수가 해당 서비스를 호출하는 클라이언트의 수만큼 밖에 되지 않는건가?**
