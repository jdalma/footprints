...
> **SOA vs MSA**  
> SOA : 재사용을 통한 비용 절감 (ESB를 통한 서비스 공유 최대화)  
...
> 여기서의 "구체화된 뷰"는 데이터베이스의 구체화 뷰가아니라 일반적인 테이블을 의미한다.

CQS가 객체의 메소드인 코드 레벨의 책임 분리라면 CQRS는 아키텍처 패턴으로 다양하게 적용할 수 있다.  
많은 테이블 조인을 피하기 위해 조회에 최적화시킨 테이블을 분리해서 설계하고 커맨드 처리를 완료한 후 발행한 도메인 이벤트에서 필요한 속성을 복사해 조회 전용 테이블에 한번 더 CUD를 실행하는 것 뿐이다.  
이 조회 전용 테이블을 동기화시키는 메커니즘을 직접 구현해서 조회 전용 데이터를 관리할 수 있다.  
  
애그리게이트에 처리를 위임하는 애플리케이션 서비스가 동일 트랜잭션에서 구체화된 뷰를 생성할 수 있지만 이벤트 주도 아키텍처는 애그리게이트가 발행한 도메인 이벤트에 반응하는 이벤트 핸들러가 구체화된 뷰에 상태를 갱신한다.  

1. 애그리게이트가 도메인 이벤트를 발행
2. 이벤트 핸들러(구체화된 뷰와 관련된 작업을 수행하기 때문에 뷰 핸들러)가 이벤트에 반응해 뷰 객체로 변환
3. ViewStore는 뷰 객체를 구체화된 뷰 테이블과 매핑한 JPA 엔티티 객체로 변환해 데이터베이스에 저장

일반적으로 구체화된 뷰 테이블을 뷰 클래스와 일대일로 매핑한다.  
명령은 Command를 쓰듯이 조회를 위한 클래스도 접두어로 Query를 사용한다.  

# 이벤트 소싱과 뷰 일관성

이벤트 소싱에서 비즈니스 로직은 애그리게이트가 처리하고 테이블들을 조인해 조회하는 책임을 CQRS로 분리한다.  
CQRS에서 조회 전용 데이터를 조작하는 이벤트 핸들러는 원칙적으로 비동기로 반응해야 한다.  
