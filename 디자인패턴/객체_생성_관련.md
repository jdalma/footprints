...
예를 들어, **인메모리로 검색 키워드에 대한 키워드가 검색된 횟수, 검색 시간, 키워드 등이 관리되어야 한다고 생각해보자.**  
`A 시스템`이 시작될 때 데이터베이스의 모든 데이터를 인메모리에 적재해야 한다.  
데이터베이스의 데이터를 업데이트하는 `B 시스템`이 있다.(삭제하는 경우는 없다고 가정한다.)  
`시스템 A`는 데이터의 실시간 특성을 보장하기 위해 데이터를 정기적으로 읽어 메모리의 데이터를 업데이트 해야하는데, **시스템 A는 현재 메모리 데이터의 마지막 업데이트 시간 `T`를 기록** 하고, `T`보다 큰 검색 시간을 가지는 모든 검색 키워드를 **추출** 한다.  
메모리에 오래된 데이터와 데이터베이스에 저장된 가장 최근 데이터에서 **달라진 정보** 를 확인하여 해당 정보만 업데이트한다.  
  
하지만 인메모리에 있는 데이터를 업데이트할 떄 신경써야 할 것이 있다.  
1. 원본 데이터를 업데이트하는 시점에 조회를 한다면?
2. 10만개 정도되는 데이터를 매번 데이터베이스에서 조회해서 해시 값을 계산하는 것이 효율적일까?

인메모리의 갱신이 되지 않는 정보도 갱신하는 것은 불필요한 연산이기 때문에 갱신이 되는 정보만 깊은 복사를 통해 기존에 참조되는 데이터와 격리하는 방법이 좋을 것이다.  
