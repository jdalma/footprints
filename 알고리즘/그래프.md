...
fun main() {
    ...
    println(Solution().minCostConnectPoints(input))
}
class Solution {
    data class Edge (
        val from: Int,
    ...
    fun minCostConnectPoints(points: Array<IntArray>): Int {
        return prim(points, 0).sum()
    }

    private fun prim(points: Array<IntArray>, node: Int) : IntArray{
        val distTo = IntArray(points.size) { Integer.MAX_VALUE }
        val visited = BooleanArray(points.size)
        val adjList = makeAdjList(points)
        val pq = TreeSet<Edge>()

        distTo[node] = 0
        pq.add(Edge(node, 0, 0))

        while (pq.isNotEmpty()) {
            val (from, to ,dist) = pq.pollFirst()!!
            visited[from] = true
            for (near in adjList[from]) {
                if (visited[near.to]) {
                    continue
                }

                if (near.dist < distTo[near.to]) {
                    distTo[near.to] = near.dist
                    pq.add(Edge(near.to, 0, near.dist))
                }
            }
        }
        return distTo
    ...
    private fun makeAdjList(points: Array<IntArray>) : List<MutableList<Edge>> {
        val edges = mutableListOf<MutableList<Edge>>()
        for (fromIndex in points.indices) {
            val from = points[fromIndex]
            edges.add(mutableListOf())
