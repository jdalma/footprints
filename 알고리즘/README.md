...
> 서로 중첩되는 부분이 없는 트리들의 집합을 **숲 (forest)**라고 한다. 

예를 들어, `V`개의 정점을 가지는 그래프 `G`는 다음의 다섯 가지 조건을 만족하는 경우 트리가 된다.
1. `G`는 `V-1`개의 간선을 가지고 순환 경로는 없다.
2. `G`는 `V-1`개의 간선을 가지고 연결되어 있다.
3. `G`는 연결되어 있지만, 어떤 간선이든 하나를 제거하면 연결이 끊어진다.
4. `G`는 비순환 그래프지만, 어떤 두 정점이든 간선을 하나 추가하면 순환 경로가 생긴다.
5. `G`는 임의의 두 정점을 연결하는 경로는 모든 경우에 있어 정확히 하나만 존재한다.
...
- 문자열
  ...
  - 트라이
    - 단일 분기 트라이
    - 3-중 탐색 트라이
      - 비교할 단어가 없으면 모두 mid로 들어간다.
      - Node 배열을 미리 만들어놓는게 아니라 각 문자 한 개씩 비교하는 방법이다.
  - 부분 문자열 탐색
    - 완전 탐색 부분 문자열 탐색
    - 커누스-모리스-프랫(KMP) 부분 문자열 탐색
      - DFA 시뮬레이션
